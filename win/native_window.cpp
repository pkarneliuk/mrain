...
void NativeWindow::create_window(AppWindow* win, unsigned int parent_id,
                                 int width, int height)
{
    ...
    else
    {
        ...
        if(AppWindow::screensaver == win->mode)
        {
            ...
            ShowCursor(FALSE);
            SystemParametersInfo(SPI_SETSCREENSAVERRUNNING, 1, NULL,
                                 SPIF_SENDCHANGE);
        }
    ...
    hwnd = CreateWindowEx(exstyle, win_class_name, AppWindow::caption, style, x,
                          y, width, height, parent, NULL, hInstance, win);
}

void NativeWindow::activate() { ShowWindow(hwnd, SW_SHOW); }

LRESULT NativeWindow::ScreenSaverProc(HWND hWnd, UINT msg, WPARAM wParam,
                                      LPARAM lParam)
{
    ...
    case WM_DESTROY:
        ...
        if(AppWindow::screensaver == win->mode)
        {
            SystemParametersInfo(SPI_SETSCREENSAVERRUNNING, 0, NULL,
                                 SPIF_SENDCHANGE);
            ShowCursor(TRUE);
        }
        break;
    ...
    case WM_ACTIVATEAPP:
        //  case WM_NCACTIVATE:
        if(FALSE == wParam && win->mode == AppWindow::screensaver)
            PostQuitMessage(0);
        break;

    case WM_SIZE: win->resize(LOWORD(lParam), HIWORD(lParam)); break;
    ...
    case WM_MOVING:
        if(win->mode == AppWindow::standalone)
            return TRUE;
        break;
    ...
    case WM_MOUSEMOVE:
    {
        ...
        if(win->start_point.x == x && win->start_point.y == y)
            break;
    }
    case WM_LBUTTONDOWN:
    case WM_RBUTTONDOWN:
    ...
    case WM_KEYDOWN:
        ...
        if(wParam != VK_ESCAPE && win->mode != AppWindow::screensaver)
            break;
    case WM_CLOSE:
    ...
    case WM_TIMER: win->repaint(); break;
...
{
    HWND hwnd = (HWND)parent_id;
    // Special case -1: use Desktop window as a parent
    if(-1 == parent_id)
        hwnd = GetDesktopWindow();
    return DialogBoxParam(NULL, MAKEINTRESOURCE(IDD_CONFIG), hwnd, DialogProc,
                          (LPARAM)this) < 1;
}

INT_PTR CALLBACK Config::DialogProc(HWND hwnd, UINT msg, WPARAM wParam,
                                    LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        {
            int id =
                (int)SendDlgItemMessage(hwnd, IDC_DEVICE, CB_ADDSTRING, 0,
                                        (LPARAM) "Capture Device not found");
        ...
        else
        {
            ...
            for(unsigned int i = 0; i < count; i++) {
                int id = (int)SendDlgItemMessage(hwnd, IDC_DEVICE, CB_ADDSTRING,
                                                 0, (LPARAM)buffers[i]);
        ...
        SendDlgItemMessage(
            hwnd, IDC_NOSHADERS, BM_SETCHECK,
            cfg->options[Options::no_shaders] ? BST_CHECKED : BST_UNCHECKED, 0);
        SendDlgItemMessage(
            hwnd, IDC_VFLIP, BM_SETCHECK,
            cfg->options[Options::vflip] ? BST_CHECKED : BST_UNCHECKED, 0);
        SendDlgItemMessage(
            hwnd, IDC_HFLIP, BM_SETCHECK,
            cfg->options[Options::hflip] ? BST_CHECKED : BST_UNCHECKED, 0);
    ...
    case WM_COMMAND:
        ...
        {
            int id =
                (int)SendDlgItemMessage(hwnd, IDC_DEVICE, CB_GETCURSEL, 0, 0);
            SendDlgItemMessage(hwnd, IDC_DEVICE, CB_GETITEMDATA, id, 0);
            // save options
            char buffer[128] = {0};
            ...
            cfg->options[Options::no_shaders] =
                BST_CHECKED == IsDlgButtonChecked(hwnd, IDC_NOSHADERS);
            cfg->options[Options::vflip] =
                BST_CHECKED == IsDlgButtonChecked(hwnd, IDC_VFLIP);
            cfg->options[Options::hflip] =
                BST_CHECKED == IsDlgButtonChecked(hwnd, IDC_HFLIP);
            cfg->options.save();
            EndDialog(hwnd, TRUE);
            return (INT_PTR)TRUE;
        }
